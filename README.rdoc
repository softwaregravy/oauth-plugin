...
Before you start, be aware that you should have an authentication system already in place (i.e. Authlogic, Devise).  
...
There are a set of methods that need to be included as part of your ApplicationController.  Furthermore, you need to make sure you make some methods available that oauth-plugin assumes are there which neither Authlogic nor Devise provide by default.

First, include the methods in your ApplicationController:
...
And then make sure logged_in? and login_required are available.  

With Devise, I used:
  alias :logged_in? :user_signed_in?
  alias :login_required :authenticate_user

With Authlogic, I used:
  def logged_in? 
    return !current_user.nil? 
  end 
  alias :login_required :require_user

=== Client OAuth Keys

At the time of this writing, the provider generator did not produce the views and controller for managing client oauth keys.  (I'm guessing this is because it's using dynamic_form which was deprecated, and removing the auto-generated functionality was the path of least resistance.)  The output of the provider generator for Rails 2 can be found here:  https://github.com/softwaregravy/oauth_provider/commit/8773cf92e6288f699e9a13fbf771d446e3245abf  I simply copied the auth_clients_controller.rb, the views/oauth_clients to my Rails 3 project and added the following line to my routes.rb file

  resources :oauth_clients

After migrating, you should be able to start up the server and visit http://localhost:3000/oauth_clients  and issue yourself a key.  
